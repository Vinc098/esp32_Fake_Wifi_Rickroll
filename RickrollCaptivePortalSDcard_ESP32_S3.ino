...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Automatically redirect to video after accepting
...
public:
  ...
  bool canHandle(AsyncWebServerRequest *request) {
    return true; // Respond to all requests
  }
...
void setupServer() {
  ...
  server.on("/video", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    if (SD.exists("/RickRoll.mp4")) {
      File videoFile = SD.open("/RickRoll.mp4");
      if (videoFile) {
        request->send(SD, "/RickRoll.mp4", "video/mp4");
        videoFile.close();
      } else {
